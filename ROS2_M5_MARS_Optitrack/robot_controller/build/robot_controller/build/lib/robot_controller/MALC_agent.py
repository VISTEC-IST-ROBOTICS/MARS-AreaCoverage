...
def get_cell(robot_agent):
    """Return current (col, row) based on current position"""
    col, row = robot_agent.get_col_row()
    return (col, row)
...
class SweepingAgent:
    TIMESTEP = 0.001
    PING_TIMEOUT = TIMESTEP * 20
    WAIT_TIME =  TIMESTEP * 20
    CASCADE_BLOCKER_PING_TIMEOUT = TIMESTEP * 100
    ...
    def update(self):
        if self.state == AgentState.INIT:
            ...
            self.robot.subcell_orientation()  # set robot center of the cell
    ...
    def swap_position(self):
        ...
        swap_agent_pos = swap_agent_data['data'].get('mega_cell')
        ...
        if self.motion_idx < len(self.path) - 1:
            if swap_agent_flag:
                ...
                self.motion_idx += 1
                col, row = self.path[self.motion_idx]
                self.robot.set_target(col, row)
        else:
    ...
    def read_msg(self,msg):
        ...
        if instruction == 'PING':
            ...
            self.agent_logger.info(f"PING_RETURN")
            self.agent_logger.info(msg)
