...
class RobotNav(Node):
    def __init__(self):
        ...
        self.declare_parameter('grid_config_path', '')

        self.namespace = self.get_parameter('robot_namespace').get_parameter_value().string_value
        config_path = self.get_parameter('grid_config_path').get_parameter_value().string_value

        self.load_grid_config(config_path)
        ...
        self.target_pose_pub = self.create_publisher(PoseStamped, f'/{self.namespace}/target_pose', 10)

        self.tf_broadcaster = tf2_ros.TransformBroadcaster(self)
        self.get_logger().info(f'RobotNav node started for namespace: {self.namespace}')
        self.pose = None
    ...
    def control_loop(self):
        ...
        if self.state == 'RUN':
            self.state = 'INIT'
            self.get_logger().info(f"STATE: INIT")
        elif self.state == 'INIT':
            self.subcell_pose_orientation()
            x_err,y_err,yaw_err = self.get_pose_error()
            if np.abs(x_err) < 0.05 and np.abs(y_err) < 0.05 and yaw_err < np.deg2rad(10):
                self.state = 'CHOOSE_DIRECTION'
                self.get_logger().info(f"STATE: CHOOSE_DIRECTION")
            else:
                pass
                #self.get_logger().info(f"INIT: wait for pose error {x_err,y_err,yaw_err}")

        elif self.state == 'CHOOSE_DIRECTION':
            self.choose_direction()
            self.state = 'MOVE'
            self.get_logger().info(f"STATE: MOVE")
        elif self.state == 'MOVE':
            if self.path_idx < len(self.path )-1:
                col, row = self.path[self.path_idx]
                cell_x, cell_y = self.get_cell_center(col, row)
                next_col, next_row = self.path[self.path_idx+1]
                #self.get_logger().info(f"path_idx,{self.path_idx}, next_col, next_row: {next_col, next_row}")
                next_cell_x, next_cell_y = self.get_cell_center(next_col, next_row)
                target_yaw = np.arctan2(next_cell_y - cell_y, next_cell_x - cell_x)
                self.target_pose = (next_cell_x, next_cell_y, target_yaw)
                self.publish_target_pose(next_cell_x, next_cell_y, yaw=target_yaw)
                #self.get_logger().info(f" target_pose: {next_cell_x:.2f}, {next_cell_y:.2f}, {np.rad2deg(target_yaw):.2f} degs")
                x_err, y_err, yaw_err = self.get_pose_error()

                #self.get_logger().info(f" x_err, y_err, yaw_err: { x_err:.2f}, {y_err:.2f}, {yaw_err:.2f}")
                if np.abs(x_err) < 0.05 and np.abs(y_err) < 0.05 and yaw_err < np.deg2rad(10):
                    self.path_idx += 1
                    self.get_logger().info(f"Move to next segment")
            else:
                self.state = 'INIT'
                self.get_logger().info(f"STATE: INIT")
    ...
    def choose_direction(self):
        ...
        curr_heading = subcell_heading[subcell_idx]
        ...
        def rotate_list(lst, n):
            return lst[n:] + lst[:n]
        heading_rotate_num = heading_to_rotate_num[curr_heading]
        dir_priority = rotate_list(dir_priority, heading_rotate_num)
        ...
        mcol, mrow = get_mega_cell(col, row)
        choosen_direction = 'STOP'
        rel_dir = ('LEFT', 'FRONT', 'RIGHT', 'BACK')
        for idx, move_dir in enumerate(dir_priority):
            offset_col, offset_row = dir_to_offset[move_dir]

            nxt_mcell = (mcol + offset_col, mrow + offset_row)
            if not self.is_mcell_obstruct(nxt_mcell) and nxt_mcell not in self.swept_cell:
                choosen_direction = rel_dir[idx]
                break


        self.get_logger().info(f"choosen_direction: {choosen_direction}")
        # Building kernel
        kernel = []
        for d_row in [-1, 0, 1]:
            for d_col in [-1, 0, 1]:
                kernel.append((mcol + d_col, mrow+ d_row))

        kernel_observed = self.observe_kernel(kernel)
        self.get_logger().info(f"kernel_observed: {kernel_observed}")


        critical = self.critical_ckeck(kernel_observed)
        ...
        if critical:
            self.get_logger().info(f"Critical, traveling...")
            sub_cell_travel_path = {
                ...
                "STOP": [(0, 0)],  # stop at final cell
            }
            rel_path = sub_cell_travel_path[choosen_direction]
            abs_path = []
            for subcell in rel_path:
                for _ in range(heading_rotate_num):
                    subcell = subcell_rotate[subcell]
                abs_cell = (mcol * 2 + subcell[0], mrow * 2 + subcell[1])
                abs_path.append(abs_cell)
            self.get_logger().info(f"abs_path, {abs_path}")
            self.path = abs_path
            self.path_idx = 0
        else:
            self.get_logger().info(f"Not critical, sweeping...")
            self.swept_cell.append((mcol, mrow ))
            ...
            rel_path = sub_cell_coverage_path[choosen_direction]
            abs_path = []
            for subcell in rel_path:
                for _ in range(heading_rotate_num):
                    subcell = subcell_rotate[subcell]
    ...
    def draw_robot(self, pose):
        ...
        screen_pos = self.world_to_screen(x, y)
        pygame.draw.circle(self.screen, (255, 0, 0), screen_pos, 20)
        ...
        pygame.draw.line(self.screen, (255, 0, 0), screen_pos, heading_pos, 2)
