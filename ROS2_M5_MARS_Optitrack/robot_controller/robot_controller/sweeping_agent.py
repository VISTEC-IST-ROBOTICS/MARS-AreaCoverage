...
class Robot:
    def __init__(self, parent_node: "SweepingAgentNode"):
        ...
        self.target_xy = (None,None)

    def get_col_row(self,initial = False):
        if self.parent_node.pose is None:
            return None, None  # Consistent return type
        robot_x, robot_y = self.target_xy
        if initial or robot_x is None or robot_y is None:
    ...
    def set_target(self,crr_col,crr_row,nxt_col,nxt_row):
        ...
        self.target_xy = next_cell_x, next_cell_y,

    def update(self):

        pass
    def mega_cell_observation(self,cell):
        if self.parent_node.is_mcell_obstruct(cell):
            return 1
        return 0
    ...
    def sweep(self):
        ...
        try:
            ...
            ros_msg = String()
            ros_msg.data = json_str
            self.parent_node.sweep_pub.publish(ros_msg)
            self.get_logger().info(f"Agent {self.id} sent sweep action to environment")
        except (TypeError, ValueError) as e:
            self.get_logger().error(f"Failed to serialize message: {e}")
...
class SweepingAgentNode(Node):
    ...
    def __init__(self):
        print ('before node  init')
        super().__init__('sweeping_agent')
        print('after node init')
        self.state = 'stopped'
        self.get_logger().info(f"Sweeping aagent init.....")
        ...
        self.sweep_pub = self.create_publisher(String, '/mars_env/sweep', 10)
        self.sweeping_agent = SweepingAgent(self)
        self._running = True  # ✅ Define before the thread starts
        #self.create_timer(5, self.update_loop)
        self.update_thread = threading.Thread(target=self.update_loop_thread, daemon=True)
        self.update_thread.start()
        self.create_subscription(String, '/mars_env/agent_control', self.handle_agent_control, 10)


    def handle_agent_control(self, msg: String):
        command = msg.data.strip().lower()

        if command == 'started':
            self.get_logger().info("Agent start")
            ...
            self.get_logger().info("Agent paused")
            ...
            self.get_logger().info("Agent stop")
            self.agent_running = False
            self.agent_paused = False
            self.state = command

            self.sweeping_agent = SweepingAgent(self) # reset agent
            # Add your stop/reset logic here
    ...
    def update_loop_thread(self):
        while self._running and rclpy.ok():
            if self.state == 'started':
                ...
                time.sleep(0.1)  # ~20 Hz or adjust as needed
    ...
    def load_agent_config(self, agent_config):
        ...
        self.get_logger().info(f"Looking for agent config: {agent_name}")
