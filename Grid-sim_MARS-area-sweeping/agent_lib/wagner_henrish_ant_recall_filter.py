...
class WagnerHenrishAntRecallFilterSweep:
    ...
    def handle_critical_cell(self, position, heading, unswept_moves):
        ...
        print(self.agent.agent_id,self.agent.agent_color, 'critical-swept',(pos_tuple in self.visited_cells),len(self.cell_directions[pos_tuple]), len(unswept_moves))
        if  (pos_tuple in self.visited_cells) and  len(self.cell_directions[pos_tuple]) == 1 and len(unswept_moves) == 2:
            self.sweep_and_reset(position)
