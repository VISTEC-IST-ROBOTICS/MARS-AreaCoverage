...
if os.path.exists(experiment_folder):
    for alg in algorithms:
        results[alg] = {'efficiency': [], 'efficiency_sd': [], 'time': [], 'time_sd': []}
        ...
        for comm_range in range_list:
            ...
            if os.path.exists(folder_path):
                ...
                for file_name, content in all_yaml_files.items():
                    ...
                    sum_swept_count = sum(agent['swept_count'] for agent in agents_statistic.values())
...
plt.title("Task Efficiency by Communication Range")
...
plt.title("Time to Completion by Communication Range")
plt.legend()
plt.tight_layout()
