...
# Example usage
folder_path = "C:\\Users\\mengi\\Documents\\GitHub\\grid-evn_MARS-area-sweeping\\gsr_recall_experiment_8agent_10comrange_40x40map"  # Replace with your folder path
...
for file_name, content in all_yaml_files.items():
    ...
    task_eff_list1.append(task_eff)
    step_required_list1.append(max(max_move_list))
#
...
for file_name, content in all_yaml_files.items():
    ...
    for agent_id in agent_id_list:
        ...
        print(agents_statistic[agent_id])
    task_eff = task_cells_num/sum_swept_count*100
    task_eff_list2.append(task_eff)
    step_required_list2.append(max(max_move_list))

plt.hist(task_eff_list1, bins=30, edgecolor='black', alpha=0.7,label = 'GSR-Recall')
plt.hist(task_eff_list2, bins=30, edgecolor='black', alpha=0.7,label = 'GSR')
plt.title('Task efficiency %')
plt.xlabel('Task efficiency %')
plt.legend()
plt.figure()
plt.hist(step_required_list1, bins=30, edgecolor='black', alpha=0.7,label = 'GSR-Recall')
plt.hist(step_required_list2, bins=30, edgecolor='black', alpha=0.7,label = 'GSR')
